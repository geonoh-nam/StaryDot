...
export function DebugJump({ onJump, onTab, onReset, contentUp, breaks = [] }) {
      ...
      {open ? (
        <>
          ...
          <View style={styles.debugPanel}>
            ...
            <Text style={styles.debugGroup}>브레이크(문항)</Text>
            <View style={styles.debugChips}>
              {breaks.map((b) => (
                <TouchableOpacity key={b.id} style={styles.debugChip}
                                  onPress={() => { setOpen(false); onJump(`break:${b.id}`); }}>
                  <Text style={styles.debugChipText}>{b.title}</Text>
                </TouchableOpacity>
              ))}
            </View>
...
const styles = StyleSheet.create({
  debugWrap: {
    position: 'absolute',
    // 좌상단은 화면마다 헤더가 덮는다 — elevation 을 올려도 헤더 밑으로 깔려 눌리지
    // 않았다. 어느 화면에서나 비어 있는 좌하단에 두고 패널을 위로 편다.
    bottom: 8,
    left: 8,
